            ...
            async first(colName) {
                ...
                if (query.toUpperCase().includes('COUNT(') ||
                    ...
                    if (tableName === 'practice_sessions') {
                        const userSessions = rows.filter((r) => {
                        ...
                        const avgAccuracy = userSessions.reduce((sum, session) => {
            ...
            async all() {
                ...
                if (query.toUpperCase().includes('WHERE') && boundValues.length > 0) {
                    ...
                    if (whereMatch) {
                        ...
                        rows = rows.filter((r) => r[field] === boundValues[0]);
                    }
                }
                // Handle SELECT DISTINCT
                if (query.toUpperCase().includes('SELECT DISTINCT')) {
                    const distinctMatch = query.match(/SELECT\s+DISTINCT\s+([^F]+)\s+as\s+(\w+)/i);
                    if (distinctMatch) {
            ...
            async run() {
                ...
                if (query.toUpperCase().includes('UPDATE')) {
                    ...
                    if (data.length > 0 && boundValues.length > 0) {
                        ...
                        const recordIndex = data.findIndex((r) => r.id === id || r.user_id === id);
                        if (recordIndex >= 0) {
