...
const mapLogbookEntry = (entry: any, user: any) => ({
...
const mapGoal = (goal: any, user: any) => ({
...
const Query = {
  logbookEntry: async (_: any, { id }: any, context: any) => {
    if (!context.user) {
  ...
  myLogbookEntries: async (
    ...
    let params: any[] = [context.user.id]
    ...
    const countResult = await context.env.DB.prepare(
      `SELECT COUNT(*) as count FROM logbook_entries ${whereClause}`
    )
      .bind(...params)
      .first()
    ...
    const edges = entries.map((entry: any, index: number) => ({
  ...
  goal: async (_: any, { id }: any, context: any) => {
    if (!context.user) {
      throw new Error('Authentication required')
    }
  ...
  myGoals: async (
    ...
    const edges = goals.map((goal: any, index: number) => ({
      cursor: Buffer.from(`${offset + index}`).toString('base64'),
...
const Mutation = {
  createLogbookEntry: async (_: any, { input }: any, context: any) => {
    if (!context.user) {
      throw new Error('Authentication required')
    }
  ...
  updateLogbookEntry: async (_: any, { input }: any, context: any) => {
    if (!context.user) {
      throw new Error('Authentication required')
    }

    // Check if entry exists
    const existingEntry = await context.env.DB.prepare(
    ...
    Object.entries(input).forEach(([key, value]) => {
      ...
      switch (key) {
        case 'timestamp':
          ...
          params.push(new Date(value as string).getTime())
          break
        case 'pieces':
        case 'techniques':
        case 'goalIds':
  ...
  deleteLogbookEntry: async (_: any, { id }: any, context: any) => {
    if (!context.user) {
      throw new Error('Authentication required')
    }
  ...
  createGoal: async (_: any, { input }: any, context: any) => {
    ...
    const milestonesWithIds = input.milestones.map((milestone: any) => ({
      ...milestone,
  ...
  updateGoal: async (_: any, { input }: any, context: any) => {
    if (!context.user) {
      throw new Error('Authentication required')
    }

    // Check if goal exists
    const existingGoal = await context.env.DB.prepare(
      'SELECT id, status, progress FROM goals WHERE id = ? AND user_id = ?'
    )
      .bind(input.id, context.user.id)
      .first()
    ...
    Object.entries(input).forEach(([key, value]) => {
      ...
      switch (key) {
        ...
        case 'progress':
          ...
          break
        case 'milestones':
          const milestonesWithIds = (value as any[]).map(milestone => ({
            ...milestone,
            id: milestone.id || nanoid(),
          }))
    ...
    // Return updated goal
    const updatedGoal = await context.env.DB.prepare(
      'SELECT * FROM goals WHERE id = ? AND user_id = ?'
    )
      .bind(input.id, context.user.id)
      .first()
  ...
  deleteGoal: async (_: any, { id }: any, context: any) => {
    if (!context.user) {
      throw new Error('Authentication required')
    }
  ...
  linkLogbookEntryToGoal: async (
    ...
    const [entry, goal] = await Promise.all([
      context.env.DB.prepare(
        ...
        .first(),
      context.env.DB.prepare(
        'SELECT id, linked_entry_ids FROM goals WHERE id = ? AND user_id = ?'
      )
    ...
    if (!linkedEntryIds.includes(entryId)) {
      ...
      await context.env.DB.prepare(
        'UPDATE goals SET linked_entry_ids = ?, updated_at = ? WHERE id = ?'
      )
  ...
  unlinkLogbookEntryFromGoal: async (
    _: any,
    { entryId, goalId }: any,
    context: any
  ) => {
    ...
    // Update goal's linked entries
    const linkedEntryIds = JSON.parse(goal.linked_entry_ids || '[]')
    const filteredIds = linkedEntryIds.filter((id: string) => id !== entryId)
...
// Field resolvers
const LogbookEntry = {
  goals: async (parent: any, _: any, context: any) => {
    ...
    const goals = result.results || []
    return goals.map((goal: any) => mapGoal(goal, parent.user))
  },

  session: async (parent: any, _: any, context: any) => {
    if (!parent.sessionId) {
      return null
    }
...
const Goal = {
  linkedEntries: async (parent: any, _: any, context: any) => {
    ...
    const entries = result.results || []
    return entries.map((entry: any) => mapLogbookEntry(entry, parent.user))
  },
}
