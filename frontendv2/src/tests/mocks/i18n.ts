...
export const i18nMock = {
  useTranslation: () => ({
    t: (key: string, options?: Record<string, unknown>) => {
      ...
      if (key === 'reports:planning.confirmDelete') return 'Are you sure you want to delete this plan?'
      if (key === 'reports:planning.emptyState.title') return 'No practice plans yet'
      if (key === 'reports:planning.emptyState.description') return 'Create your first practice plan to get started'
      if (key === 'reports:planning.emptyState.createButton') return 'Create Your First Plan'
      ...
      if (key === 'reports:planningEditor.planTitlePlaceholder') return 'Enter plan title'
      if (key === 'reports:planningEditor.description') return 'Description'
      if (key === 'reports:planningEditor.descriptionPlaceholder') return 'Describe your practice plan'
      ...
      if (key === 'reports:planningEditor.flexibility.fixed') return 'Fixed Time'
      if (key === 'reports:planningEditor.flexibility.sameDay') return 'Same Day'
      ...
      if (key === 'reports:planningEditor.removeSegment') return 'Remove Segment'
      ...
      if (key === 'reports:planningEditor.segmentInstructions') return 'Instructions'
      if (key === 'reports:planningEditor.techniques') return 'Techniques'
      if (key === 'reports:planningEditor.reflectionPrompts') return 'Reflection Prompts'
      ...
      if (key === 'reports:planningEditor.validation.titleRequired') return 'Title is required'
      if (key === 'reports:planningEditor.validation.segmentRequired') return 'At least one segment is required'
      if (key === 'reports:planningEditor.validation.invalidSchedule') return 'Invalid schedule'

      // Check-in modal translations
      if (key === 'reports:planningCheckIn.title') return 'Check in and log practice'
      if (key === 'reports:planningCheckIn.upcomingSession') return 'Next session details'
      if (key === 'reports:planningCheckIn.actualDuration') return 'Actual duration (minutes)'
      if (key === 'reports:planningCheckIn.notes') return 'Notes'
      if (key === 'reports:planningCheckIn.notesPlaceholder') return 'What stood out about this session?'
      if (key === 'reports:planningCheckIn.reflection') return 'Reflection'
      if (key === 'reports:planningCheckIn.noPrompts') return 'No prompts configured for this session.'
      if (key === 'reports:planningCheckIn.promptPlaceholder') return 'Add your thoughts here'
      if (key === 'reports:planningCheckIn.complete') return 'Check off'
      if (key === 'reports:planningCheckIn.cancel') return 'Cancel'
      if (key === 'reports:planningCheckIn.defaultNote') return 'Logged from practice plan'
      if (key === 'reports:planningCheckIn.genericError') return 'Unable to complete check-in'
      ...
      if (key === 'reports:planningView.occurrence.scheduled') return 'Scheduled'
      if (key === 'reports:planningView.occurrence.completed') return 'Completed'
