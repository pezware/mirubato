...
describe('Search Handler', () => {
  ...
  beforeEach(() => {
    ...
    // Reset mock methods
    mockDbMethods.search.mockReset().mockResolvedValue({ results: [], total: 0 })
    mockDbMethods.searchTerms.mockReset().mockResolvedValue({ results: [], total: 0 })
    mockDbMethods.searchByType.mockReset().mockResolvedValue({ results: [], total: 0 })
    ...
    mockDbMethods.searchByEmbedding.mockReset().mockResolvedValue({ results: [], total: 0 })
  ...
  describe('GET /api/v1/search', () => {
    it('should return search results from cache if available', async () => {
      const CacheService = (await import('../../services/storage/cache-service')).CacheService
      ...
      const response = await testHandler(searchHandler, request, mockEnv, mockExecutionContext)
    ...
    it('should support pagination', async () => {
      const DictionaryDatabase = (await import('../../services/storage/dictionary-database')).DictionaryDatabase
  ...
  describe('POST /api/v1/search/semantic', () => {
    it('should perform semantic search', async () => {
      const CloudflareAIService = (await import('../../services/ai/cloudflare-ai-service')).CloudflareAIService
      const DictionaryDatabase = (await import('../../services/storage/dictionary-database')).DictionaryDatabase
      ...
      mockDbMethods.getEntriesWithEmbeddings.mockResolvedValue(mockEntriesWithEmbeddings)
  ...
  describe('GET /api/v1/search/related', () => {
    it('should return related searches', async () => {
      ...
      const relatedTerms = ['pianoforte', 'keyboard', 'grand piano', 'upright piano']
