...
IMPORTANT: Respond with valid JSON only. No markdown, no additional text, just the JSON object.`
      ...
      await this.logUsage({
        ...
        total_tokens: this.estimateTokens(structuredPrompt) + this.estimateTokens(response.response || ''),
        latency_ms: latency,
   ...
   */
  private async logUsage(usage: Omit<AIModelUsage, 'id' | 'created_at'>): Promise<void> {
    try {
      // Calculate approximate cost
      const modelConfig = Object.values(CLOUDFLARE_AI_MODELS.TEXT_GENERATION)
