...
app.get('/fredericchopin/dashboard', async c => {
...
<body>
    ...
    <script>
        // Get auth info from URL params or localStorage
        const urlParams = new URLSearchParams(window.location.search);
        const authToken = urlParams.get('token') || localStorage.getItem('auth-token');
        const userEmail = urlParams.get('email') || localStorage.getItem('user-email');
        ...
        if (!authToken) {
            ...
            if (urlParams.get('token')) {
                ...
                // Clean URL
                window.history.replaceState({}, document.title, '/fredericchopin/dashboard');
            }
