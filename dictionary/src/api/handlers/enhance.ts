  ...
  if (request.mode === 'single') {
    ...
    return c.json(createApiResponse({
      ...
      improvement: {
        ...
        score_after: enhancedEntry?.quality_score.overall || entry.quality_score.overall,
    ...
    if (criteria.max_quality_score !== undefined) {
      filtered = filtered.filter(e => e.quality_score.overall <= criteria.max_quality_score!)
    }
    ...
    return c.json(createApiResponse({
      job_id: jobId,
      status: 'queued',
      estimated_completion: new Date(Date.now() + filtered.length * 2000).toISOString(), // ~2s per term
  ...
  for (const entry of entries) {
    try {
      ...
      jobData.improvements.push({
        ...
        changes_made: [
          scoreAfter - scoreBefore > 20 ? 'Major improvements' : 'Minor improvements',
          enhancedEntry.definition.etymology && !entry.definition.etymology ? 'Added etymology' : null,
          enhancedEntry.definition.pronunciation && !entry.definition.pronunciation ? 'Added pronunciation' : null,
          Object.keys(enhancedEntry.references).length > Object.keys(entry.references).length ? 'Added references' : null
        ].filter(Boolean)
      })
