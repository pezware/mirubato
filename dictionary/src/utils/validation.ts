...
import { z } from 'zod'
import { TermType, SearchSource, FeedbackType } from '../types/dictionary'
...
export const termTypeSchema = z.enum([
  'instrument', 'genre', 'technique', 'composer', 'theory', 'general'
] as const)
...
export const feedbackTypeSchema = z.enum([
  'accuracy', 'clarity', 'completeness', 'other'
] as const)
...
export const searchQuerySchema = z.object({
  ...
  limit: z.coerce.number().int().min(1).max(100).default(20),
  offset: z.coerce.number().int().min(0).default(0),
  sort_by: z.enum(['relevance', 'alphabetical', 'quality', 'popularity']).default('relevance'),
...
export const exportQuerySchema = z.object({
  ...
  min_quality: z.coerce.number().min(0).max(100).optional(),
  types: z.string().optional().transform(val => val?.split(',').filter(Boolean)),
