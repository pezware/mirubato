...
SELECT
  ...
  json_object(
    ...
    'techniques', json(COALESCE(techniques, '[]')),
    'goalIds', json(COALESCE(goal_ids, '[]')),
    'notes', COALESCE(notes, ''),
    'tags', json(COALESCE(tags, '[]')),
    'metadata', json(COALESCE(metadata, '{\"source\":\"manual\"}')),
...
SELECT
  ...
  json_object(
    ...
    'milestones', json(COALESCE(milestones, '[]')),
    'linkedEntries', json(COALESCE(linked_entries, '[]')),
    'createdAt', created_at,
    'updatedAt', updated_at
  ) as data,
