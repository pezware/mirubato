  ...
  useTranslation: () => ({
    t: (key: string) => {
      const translations: Record<string, string> = {
        ...
        'timePicker.hint': 'Drag hour hand, click for minutes',
        'timePicker.confirmTime': 'Set',
        'timePicker.clickToTypeManually': 'Click to type manually',
        cancel: 'Cancel',
...
describe('ClockTimePicker', () => {
  ...
  describe('time display', () => {
    it('should show 12 for midnight', () => {
      render(<ClockTimePicker value="00:00" onChange={vi.fn()} />)
      expect(screen.getByText('12:00 AM')).toBeInTheDocument()
    })

    it('should show 12 for noon', () => {
      render(<ClockTimePicker value="12:00" onChange={vi.fn()} />)
      expect(screen.getByText('12:00 PM')).toBeInTheDocument()
    })

    it('should show AM for morning times', () => {
      render(<ClockTimePicker value="09:30" onChange={vi.fn()} />)
      expect(screen.getByText('9:30 AM')).toBeInTheDocument()
    })

    it('should show PM for afternoon times', () => {
      render(<ClockTimePicker value="15:45" onChange={vi.fn()} />)
  ...
  describe('AM/PM toggle', () => {
    ...
    it('should toggle PM to AM', async () => {
      const user = userEvent.setup()
      render(<ClockTimePicker value="15:30" onChange={vi.fn()} />)
