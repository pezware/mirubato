...
(function (TimeSignature) {
    ...
    TimeSignature["CUT_TIME"] = "2/2";
    TimeSignature["COMMON_TIME"] = "C";
...
var DynamicMarking;
(function (DynamicMarking) {
    DynamicMarking["pppp"] = "pppp";
    DynamicMarking["ppp"] = "ppp";
    DynamicMarking["pp"] = "pp";
    DynamicMarking["p"] = "p";
    DynamicMarking["mp"] = "mp";
    DynamicMarking["mf"] = "mf";
    DynamicMarking["f"] = "f";
    DynamicMarking["ff"] = "ff";
    DynamicMarking["fff"] = "fff";
    DynamicMarking["ffff"] = "ffff";
    DynamicMarking["sfz"] = "sfz";
    DynamicMarking["fp"] = "fp";
...
class MusicXMLToScoreConverter {
    ...
    constructor(xmlString) {
        this.parser = new XMLParser({
            ...
            isArray: (tagName) => {
                ...
                return ['part', 'measure', 'note', 'score-part', 'creator'].includes(tagName);
            },
        });
        ...
        if (!this.xmlData['score-partwise'] && !this.xmlData['score-timewise']) {
            throw new Error('Invalid MusicXML: must be score-partwise or score-timewise format');
        }
        // If it's score-timewise, we'll need to convert it to partwise format
        if (this.xmlData['score-timewise']) {
            console.log('   Converting from score-timewise to score-partwise format...');
     ...
     */
    convertTimewiseToPartwise(timewiseData) {
        ...
        partwise['score-partwise'].part = Array.from(partsMap.values());
        return partwise;
    }
    convert() {
    ...
    extractArranger(scoreData) {
        if (scoreData.identification?.creator) {
            const creators = Array.isArray(scoreData.identification.creator)
                ? scoreData.identification.creator
                : [scoreData.identification.creator];
            const arranger = creators.find((c) => c['@_type'] === 'arranger');
            if (arranger) {
    ...
    extractParts(scoreData) {
        ...
        scoreParts.forEach((scorePart) => {
            ...
            const partName = typeof partNameRaw === 'string'
                ? partNameRaw
    ...
    extractInstrumentFromPart(scorePart) {
        ...
        if (partNameStr.includes('piano'))
            return 'piano';
        if (partNameStr.includes('guitar'))
            return 'guitar';
        if (partNameStr.includes('violin'))
            return 'violin';
        if (partNameStr.includes('voice'))
            return 'voice';
        return 'unknown';
    }
    extractStavesForPart(scoreData, partId) {
    ...
    extractMeasures(scoreData) {
        ...
        const measureElements = Array.isArray(firstPart.measure)
            ? firstPart.measure
            : [firstPart.measure];
        measureElements.forEach((_, index) => {
            measures.push(this.extractMeasure(scoreData, index + 1));
        });
        return measures;
    }
    extractMeasure(scoreData, measureNumber) {
        const staves = [];
        const parts = Array.isArray(scoreData.part)
            ? scoreData.part
            : [scoreData.part];
    ...
    extractStavesFromMeasure(measure, partId) {
        const stavesMap = new Map();
    ...
    extractClefForStaff(measure, staffNumber) {
    ...
    extractStemDirection(notes, voiceNumber) {
        let upCount = 0;
        let downCount = 0;
        notes.forEach(note => {
    ...
    convertNote(noteData, voiceId, staffId) {
    ...
    extractMeasureAttributes(measure) {
        const attributes = {};
        if (measure.attributes?.time) {
            const time = measure.attributes.time;
            const timeSig = `${time.beats}/${time['beat-type']}`;
            if (Object.values(TimeSignature).includes(timeSig)) {
                attributes.timeSignature = timeSig;
            }
        }
        if (measure.attributes?.key?.fifths !== undefined) {
            attributes.keySignature = this.fifthsToKeySignature(measure.attributes.key.fifths);
        }
        ...
        if (measure.direction?.['direction-type']?.dynamics) {
            ...
            if (dynamicType &&
                Object.values(DynamicMarking).includes(dynamicType)) {
                attributes.dynamics = dynamicType;
            }
        }
...
async function findMainScoreFile(filepath) {
        ...
        yauzl.open(filepath, { lazyEntries: true }, (err, zipfile) => {
            ...
            zipfile.on('end', () => {
                ...
                if (mainScoreFile) {
                    ...
                    const scoreFile = entries.find(filename => filename.endsWith('.xml') && !filename.includes('META-INF/'));
                    if (scoreFile) {
...
async function readFileFromZip(filepath, filename) {
    return new Promise((resolve, reject) => {
        yauzl.open(filepath, { lazyEntries: true }, (err, zipfile) => {
            if (err) {
                reject(err);
...
function generateTypeScriptFile(score, varName) {
    ...
    const scoreString = JSON.stringify(score, (key, value) => {
    ...
    const formattedScore = scoreString
        ...
        const enumKey = Object.entries(TimeSignature).find(([_, v]) => v === value)?.[0];
        return enumKey ? `"timeSignature": TimeSignature.${enumKey}` : match;
    })
        .replace(/"keySignature": "([^"]+)"/g, (match, value) => {
        const enumKey = Object.entries(KeySignature).find(([_, v]) => v === value)?.[0];
...
async function convertMXLFile(inputPath, outputDir) {
    try {
        ...
        console.log(`   📊 ${score.measures.length} measures, ${score.parts.length} parts`);
    }
    catch (error) {
