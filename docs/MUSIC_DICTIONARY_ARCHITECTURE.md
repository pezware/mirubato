...
export class CloudflareAIService {
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    const response = await this.ai.run('@cf/baai/bge-base-en-v1.5', { text })
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a music dictionary expert. Provide accurate, educational definitions.',
          },
...
export class QualityScorer {
  ...
  private generateSuggestions(entry: DictionaryEntry, score: number): string[] {
    ...
    if (score < 70) {
      suggestions.push(
        'Consider regenerating the definition for better clarity'
      )
    }
    ...
    if (
      ...
      !entry.references.shopping?.instruments
    ) {
...
healthHandler.get('/health', async c => {
  ...
  const [database, cache, cloudflareAI, openAI, anthropic, smokeTests] =
    await Promise.all([
  ...
  try {
    ...
    if (env.AI) {
      const response = await env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
        prompt:
          'Define "forte" in music context. Response in JSON: {"definition": "..."}',
        max_tokens: 100,
        temperature: 0.3,
      })
    ...
    if (env.AI) {
      const response = await env.AI.run(
        ...
        {
          prompt:
            'Rate this definition quality from 1-10: "A piano is a keyboard instrument." Response: {"score": X}',
          max_tokens: 50,
          temperature: 0.1,
        }
      )
...
async function testAllAIModels(env: Env) {
  const models = [
    ...
    },
    {
      ...
      test: () =>
        env.AI?.run('@cf/mistral/mistral-7b-instruct-v0.2', {
          prompt: 'Test',
          max_tokens: 10,
        }),
    },
    {
      ...
      name: 'Gemma 7B',
      test: () =>
        env.AI?.run('@cf/google/gemma-7b-it', {
          prompt: 'Test',
          max_tokens: 10,
        }),
    },
